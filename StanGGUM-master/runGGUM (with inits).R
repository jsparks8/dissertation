...
write.csv(summary(GGUM1),paste0(output_dir, "/", Sys.Date(), "_GGUM1_summary_", chains, 
                                "chain_", iter, "_bmggum.csv"))

saveRDS(GGUM1, paste0(output_dir, "/", Sys.Date(), "_GGUM1_summary_", chains, 
...
g <- arrangeGrob(qplot(estimate, truth, data = (df %>% dplyr::filter(family == "a")), main="a"),
                 ...
                 qplot(estimate, truth, data = (df %>% dplyr::filter(family == "theta")), main="theta"))
#generates g
ggsave(paste0(output_dir, "/", Sys.Date(), "_GGUM1_Plot_", chains, 
